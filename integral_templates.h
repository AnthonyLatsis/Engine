...
// TEMPLATES FOR TREE CONSTRUCTION AND COMPARISON

//template for tree ax
struct TREE * proportional(int coefficient_existence) {
...
// template for tree (ax ± b)
struct TREE * linear(int coefficient_existence) {
...
// TEMPLATES FOR INTEGRALS OF ELEMENTARY FUNCTIONS

// template for constant
struct TREE * number_integral(struct TREE * tree) {
...
// template for x^a
struct TREE * variable_in_power_integral(struct TREE * tree) {
  ...
  return tree_join(&division, factor, denominator);;
}

// template for a^(bx) and a^(bx + c)
struct TREE * number_in_power_integral(struct TREE * tree) {
...
// template for sin(ax) and sin(ax + b)
struct TREE * sine_integral_LP(struct TREE * tree) {
	struct TREE * argument_derivative = differentiate(tree->right_branch);
...
// template for cos(ax) and cos(ax + b)
struct TREE * cosine_integral_LP(struct TREE * tree) {      
...
// template for 1/(cos(ax + b))^2 and 1/(cos(ax))^2
struct TREE * tan_derivative_integral_LP(struct TREE * tree) {
	return NULL;
}

// template for 1/(sin(ax + b))^2 and 1/(sin(ax))^2
struct TREE * cot_derivative_integral_LP(struct TREE * tree) {
	return NULL;     
}

// template for sh(ax) and sh(ax + b)
struct TREE * hyp_sin_integral_LP(struct TREE * tree) {
...
// template for ch(ax) and ch(ax + b)
struct TREE * hyp_cos_integral_LP(struct TREE * tree) {
	struct TREE * argument_copy = tree_copy(tree->right_branch);
...
// template for exp(ax) and exp(ax + b)
struct TREE * exponent_integral_LP(struct TREE * tree) {
	struct TREE * argument_derivative = differentiate(tree->right_branch);
...
// template for tan(ax) and tan(ax + b)
struct TREE * tangent_integral_LP(struct TREE * tree) {
	struct TREE * argument_derivative = differentiate(tree->right_branch);
...
//template for cot(ax) and cot(ax + b)
struct TREE * cotangent_integral_LP(struct TREE * tree) {
	struct TREE * Sine       = T_function(SINE, tree_copy(tree->right_branch));
...
//template for tanh(ax) and tanh(ax + b)
struct TREE * hyp_tan_integral_LP(struct TREE * tree) {
...
// template for coth(ax) and coth(ax + b)
struct TREE * hyp_cot_integral_LP(struct TREE * tree) {
...
// template for ln(ax) and ln(ax + b)
struct TREE * logarithm_integral_LP(struct TREE * tree) {
...
// template for arccos(ax), arccos(ax + b) and arcsin(ax), arcsin(ax + b)
struct TREE * inv_sin_cos_integral_LP(struct TREE * tree, char factor_symbol) {    
	struct TREE * copy = tree_copy(tree);
...
// template for arccot(ax), arccot(ax + b) and arctan(ax), arctan(ax + b)
struct TREE * inv_tan_cot_integral_LP(struct TREE * tree, char factor_symbol) {
	struct TREE * argument_derivative = differentiate(tree->right_branch);
...
//template for arccosh(ax) and arccosh(ax + b)
struct TREE * inv_hyp_sin_integral_LP(struct TREE * tree) {	
	struct TREE * copy = tree_copy(tree);
...
//template for arcsinh(ax) and arcsinh(ax + b)
struct TREE * inv_hyp_cos_integral_LP(struct TREE * tree) {
	struct TREE * copy = tree_copy(tree);
...
//template for arctanh(ax), arccoth(ax) and arctanh(ax + b), arccoth(ax + b)
struct TREE * inv_hyp_tan_cot_integral_LP(struct TREE * tree) {
	struct TREE * argument_derivative = differentiate(tree->right_branch);
