...
void number_analyze() {
  ...
  if (pointer_for_numbers == 1) {
    FUNC((input[pointer_for_input - 1] - '0'), NO_FUNCTION, NO_SYMBOL, NUMBER);
    return;
  }
...
void function_analyze() {
  ...
  if (pointer_for_functions == 1) {
    global_variable = input[pointer_for_input - 1];

    FUNC(NO_VALUE, NO_FUNCTION, input[pointer_for_input - 1], VARIABLE);
    return;
  }
...
void auxiliary_for_letters()
{
  if (pointer_for_input != ZERO)
  {
    ...
    &&  ((input[pointer_for_input - 1] >= 'a' && input[pointer_for_input - 1] <= 'z')
    ||   (input[pointer_for_input - 1] >= 'A' && input[pointer_for_input - 1] <= 'Z'))) {
      FUNC(NO_VALUE, NO_FUNCTION, MULTIPLICATION, SYMBOL); 
...
struct TREE * auxiliary(char * postfix) {
  ...
  if (strcmp(postfix, "last") == 0) tree = tree_copy(Input);

  else if (strcmp(postfix, "answer") == 0) tree = tree_copy(answer);
...
void command_derivative(char * command, char * postfix, int repetitions) {
  int box = pointer_for_input, counter = 0;
  ...
  if (strcmp(postfix, "last") == 0) infix_print(Input);

  else if (strcmp(postfix, "answer") == 0) infix_print(answer);
...
void command_system_solution() {
  ...
  for (counter = ZERO; counter < order; counter ++)
      printf("x%d = %f, ", counter + 1, matrix->variables[counter]);
